...
def get_teams(repo: TeamRepository = Depends()):
    return repo.get_all()
...
def delete_team(id: int, repo: TeamRepository = Depends()):
    repo.delete_team(id)
    return True
...
def update_team(
    id: int, team_in: TeamIn, response: Response, repo: TeamRepository = Depends()
):
