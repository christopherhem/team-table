...
def get_member(uid: int, response: Response, repo: MemberRepository = Depends()):
...
def delete_member(uid: int, repo: MemberRepository = Depends()):
    repo.delete(uid)
    return True


@router.put("/api/teams/{id}/members/{uid}", response_model=Union[Error, MemberOut])
def edit_member(uid: int, member: MemberIn, repo: MemberRepository = Depends()):
