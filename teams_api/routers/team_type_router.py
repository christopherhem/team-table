...
def get_team_types(response: Response, repo: TeamTypeRepository = Depends()):
    record = repo.get_all()
    if record is None:
...
def get_team_type(id: int, response: Response, repo: TeamTypeRepository = Depends()):
...
def delete_team_type(id: int, repo: TeamTypeRepository = Depends()):
